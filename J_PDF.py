...
for i in range(len(B_list)):
    ...
    # Get the equivalent value of Email within the list (B_List).
    g_add = B_list[i].get("Address")
    j_email = str("      Address:" + g_add)
    pdf.set_font("times", "", 12)
    pdf.cell(200, 10, txt=j_email, ln=1, align='L')
...
# Add spaces
pdf.cell(10, 10, txt=" ", ln=1, align='L')
# Set style and size of font
pdf.set_font("times", "B", 14)
# contains a text and align it in left positon
...
for i in range(len(B_list)):
    ...
    # Get the equivalent value of Bithday within the list (B_List).
    g_day = B_list[i].get("Birthay")
    j_day = str("      Birthday:" + g_day)
    pdf.set_font("times", "", 12)
    pdf.cell(200, 10, txt=j_day, ln=1, align='L')

    # Get the equivalent value of Civil Status within the list (B_List).
    g_cs = B_list[i].get("Civil Status")
    j_cs = str("      Civil Status:" + g_cs)
    pdf.set_font("times", "", 12)
    pdf.cell(200, 10, txt=j_cs, ln=1, align='L')

    # Get the equivalent value of Hobbies within the list (B_List).
    g_h = B_list[i].get("Hobbies")
    j_h = str("     Hobbies:" + g_h)
    pdf.set_font("times", "", 12)
    pdf.cell(200, 10, txt=j_h, ln=1, align='L')
